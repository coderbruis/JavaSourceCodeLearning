...
在 classic consumer 实现里，会进入 ClassicKafkaConsumer.poll()：
...
```
  ...
  **rebalance 不是后台自动完整完成的，classic consumer 的 group rebalance 主要是在用户线程调用 **`poll()`** 时被推进的。**
...
```
  ...
  这里两处判断为啥都是上次的JoinGroup？因为这是在构造当前这次JoinGroup之前的判断逻辑，所以得拿上次的JoinGroup的元数据和订阅信息和当前准备构造JoinGroup的数据进行对比。
...
位置在 GroupCoordinatorService.java ：
...
leader 的核心逻辑在 ConsumerCoordinator.onLeaderElected()：
...
核心代码在 ConsumerCoordinator.java：
...
leader 计算完 assignment 后，会在 AbstractCoordinator.java 构造 leader SyncGroup：
